...
pub fn inspect(identity: &Value, length: u64, sha256: &str) -> Inspection {
    ...
    match inspect_plan_observe(identity, &mut |free| {
        ...
        Ok(plan) => {
            ...
            result.restore_available = plan.recovery.is_some();
            if sha256 != ISO_HASH {
                result
                    .reasons
                    .push("Keeping files is currently qualified only for Omarchy 4.0.2.".into());
            }
            if plan.data.free_bytes < result.required_bytes {
                result.reasons.push(
                    "There is not enough free space for the ISO and a 64 MiB safety reserve."
                        .into(),
                );
            }
            ...
            if plan.recovery.is_some() {
                result.reasons.push("An Omarchy boot backup already exists. Restore the previous boot setup before adding another installer.".into());
            }
...
pub fn validate_addition(plan: &Plan, length: u64, sha256: &str) -> Result<(), String> {
    ...
    if plan.recovery.is_some() {
        return Err(
            "Restore the existing Omarchy boot backup before adding another installer.".into(),
        );
    }
...
fn install_files(
    ...
    if cancel.load(Ordering::Relaxed) {
        return Err("Cancelled before activating the new USB boot setup. Use Restore previous boot setup to remove preparation files.".into());
    }
    emit(
...
pub fn restore(identity: &Value, plan: &Plan) -> Result<Value, String> {
    let fresh = recheck(identity, plan)?;
    restore_files(&fresh)
}
fn restore_files(plan: &Plan) -> Result<Value, String> {
    let record = plan
        .recovery
        .as_ref()
        .ok_or("No verified USB recovery record is available")?;
    let mut guards = vec![];
    let boot = hold_path(&plan.efi.root, FALLBACK, true, &mut guards)?;
    let temp = hold_path(
        &plan.efi.root,
        "EFI/BOOT/OMARCHY.RESTORE",
        false,
        &mut guards,
    )?;
    let backup = hold_path(
        &plan.data.root,
        &backup_relative(&plan.data.root, record)?,
        false,
        &mut guards,
    )?;
    let current = optional_hash(&plan.efi.root, FALLBACK)?;
    if current.is_some()
        && current != Some(record.loader_hash.clone())
        && current != record.old_boot_hash
    {
        return Err("The USB bootloader changed. Restoration will not overwrite it.".into());
    }
    if let Some(hash) = &record.old_boot_hash {
        let bytes = read_small(&backup, 32 * 1024 * 1024)?;
        if digest(&bytes) != *hash {
            return Err("USB boot backup changed".into());
        }
        if temp.exists() {
            if digest(&read_small(&temp, 32 * 1024 * 1024)?) != *hash {
                return Err("A different file occupies the boot restoration staging path".into());
            }
        } else {
            write_new(&temp, &bytes)?;
        }
        if optional_hash(&plan.efi.root, FALLBACK)? != current {
            return Err("USB bootloader changed during restoration".into());
        }
        replace_file(&temp, &boot)?;
    } else if boot.exists() {
        if optional_hash(&plan.efi.root, FALLBACK)? != Some(record.loader_hash.clone()) {
            return Err("USB bootloader changed; restoration stopped".into());
        }
        fs::remove_file(&boot).map_err(|e| e.to_string())?;
    }
    if optional_hash(&plan.efi.root, FALLBACK)? != record.old_boot_hash {
        return Err("The restored bootloader could not be verified".into());
    }
    // Retain the ISO and recovery archive. Remove only byte-identical app boot
    // files; never delete other files from either volume.
    for (relative, hash) in [
        (CONFIG, &record.config_hash),
        ("EFI/BOOT/OMARCHY.NEW", &record.loader_hash),
    ] {
        if optional_hash(&plan.efi.root, relative)? == Some(hash.clone()) {
            let path = hold_path(&plan.efi.root, relative, false, &mut guards)?;
            fs::remove_file(path).map_err(|e| e.to_string())?;
        }
    }
    let archived_backup = hold_path(
        &plan.data.root,
        &format!("{NAMESPACE}/previous-BOOTX64-{}.EFI", record.operation),
        false,
        &mut guards,
    )?;
    if record.old_boot_hash.is_some() && backup != archived_backup {
        fs::rename(&backup, &archived_backup).map_err(|e| e.to_string())?;
    }
    let active_record = hold_path(
        &plan.data.root,
        &format!("{NAMESPACE}/record.json"),
        false,
        &mut guards,
    )?;
    let archived_record = hold_path(
        &plan.data.root,
        &format!("{NAMESPACE}/record-{}-restored.json", record.operation),
        false,
        &mut guards,
    )?;
    fs::rename(active_record, archived_record).map_err(|e| e.to_string())?;
    Ok(
        json!({"mode":"restore","backupPath":backup.parent(),"message":"The previous USB bootloader was restored and verified. The ISO, existing files and boot backup were retained. Safely eject the USB before unplugging it."}),
    )
}
...
mod tests {
    ...
    #[test]
    fn install_restore_roundtrip_preserves_user_files_and_archives_backup() {
        let dir = tempfile::tempdir().unwrap();
        let (identity, mut plan, source) = fixture(dir.path());
        install_files(
            &identity,
            &plan,
            &source,
        ...
        .unwrap();
        plan.recovery = Some(record.clone());
        restore_files(&plan).unwrap();
        assert_eq!(
            fs::read(plan.efi.root.join(FALLBACK)).unwrap(),
        ...
        assert!(!plan
            .data
            .root
            .join(format!("{NAMESPACE}/record.json"))
            .exists());
        assert!(!plan.efi.root.join(CONFIG).exists());
    }
    #[test]
    fn failed_copy_keeps_original_boot_and_has_recovery_record() {
        let dir = tempfile::tempdir().unwrap();
        let (identity, mut plan, source) = fixture(dir.path());
    ...
    #[test]
    fn refuses_restoration_after_another_program_changes_the_loader() {
        let dir = tempfile::tempdir().unwrap();
        let (identity, mut plan, source) = fixture(dir.path());
        install_files(
            &identity,
        ...
        fs::write(
            plan.efi.root.join(FALLBACK),
            b"another program's bootloader",
        )
        .unwrap();
        assert!(restore_files(&plan).is_err());
        assert_eq!(
            fs::read(plan.efi.root.join(FALLBACK)).unwrap(),
            b"another program's bootloader"
        );
    ...
    #[test]
    fn rejects_corrupted_backup_before_restoring_anything() {
        let dir = tempfile::tempdir().unwrap();
        let (identity, mut plan, source) = fixture(dir.path());
        install_files(
            &identity,
