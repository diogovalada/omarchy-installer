<script lang="ts">
  ...
  import { setup, setupActive } from './setup';
...
<section class="flow" aria-label={kind === 'usb' ? 'Create bootable USB' : 'Install without USB'}>
  <div class="heading"><h2>{kind === 'usb' ? 'Create bootable USB' : 'Install without USB'}</h2><button class="back" disabled={active} onclick={close}><ArrowLeft size={15}/>Back</button></div>
  {#if !downloads.native}
    <p>Open the desktop app to inspect disks and continue.</p>
  {:else if !readyImage && kind !== 'usb'}
    <p>Download and verify the Omarchy image above to continue.</p>
  {:else if complete}
    <div class="done" role="status"><Check size={21}/>{kind === 'usb' ? (snapshot?.receipt?.receipt?.mode === 'restore' ? 'Previous USB boot setup restored.' : 'Bootable USB created and verified.') : 'Omarchy has been installed.'}</div>
    {#if kind === 'usb'}
  ...
  {:else}
    <p>{kind === 'usb' ? 'Choose a USB drive. We’ll check its free space and boot setup so you can keep existing files where supported, or erase the drive and create a fresh installer.' : 'Choose where to make space for Omarchy and how much to allocate. Windows is retained. A boot menu will let you choose either OS at startup.'}</p>
    {#if kind === 'direct'}
    ...
    {:else}
      ...
      {:else}
        ...
        {#if selectedChoice}
          <div class="usb-options">
            ...
            {#if !readyImage}<p>Download and verify the Omarchy image above to create an installer. Restoring an existing boot backup does not need the ISO.</p>{/if}
            ...
            <div class="usb-option">
              <button class="primary" disabled={!readyImage || !selectedChoice.usb?.eligible} onclick={()=>{void setup.start(selected,undefined,undefined,undefined,'preserve');}}>Keep files and add installer</button>
              <p>Retain existing files and partitions. Add the verified installer and its UEFI boot files.</p>
              {#if selectedChoice.usb?.replacesBootloader}<p>The current UEFI bootloader will be saved on this USB before replacement. Its previous boot option will be unavailable until you restore the backup here.</p>{/if}
              {#each selectedChoice.usb?.reasons ?? [] as reason}<p class="muted">{reason}</p>{/each}
            ...
            <div class="usb-option">
              <button class="erase" disabled={!readyImage} onclick={()=>{void setup.start(selected,undefined,undefined,undefined,'erase');}}>Erase USB and create installer</button>
              <p>Deletes every file and partition on this USB. You’ll confirm the selected drive before erasing it.</p>
            </div>
            {#if selectedChoice.usb?.restoreAvailable}<div class="usb-option"><button onclick={()=>{void setup.start(selected,undefined,undefined,undefined,'restore');}}>Restore previous boot setup</button><p>Restore the saved bootloader. Keep the ISO, existing files and recovery archive.</p></div>{/if}
          </div>
        {/if}
      {/if}
      ...
      <div class="actions">
        ...
        {#if kind === 'direct'}<button onclick={() => { selected=''; void setup.inspect(kind, true); }}>Check with administrator access</button>{/if}
      </div>
    {/if}
  {/if}
