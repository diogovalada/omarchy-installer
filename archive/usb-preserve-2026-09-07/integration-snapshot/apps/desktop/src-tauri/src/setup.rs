...
// Inventory and boot restoration do not require a downloaded image. The
// executable supplies a protected filesystem for device exclusion checks.
// This placeholder never reaches image authentication or image writing.
fn usb_inspection_source(downloads: &Downloads) -> Result<SourceImage, String> {
    source(downloads).or_else(|_| {
        Ok(SourceImage {
            path: std::env::current_exe().map_err(|e| e.to_string())?,
            file_name: "unused-inspection.iso".into(),
            length: 6_227_752_960,
            sha256: "2ef8e624aa1bec7e277e28056b8535a6c9373ba48d7ede3f1a01cb6d2373cfb8".into(),
            signature: vec![],
        })
    })
}
...
pub fn inspect_setup(
    ...
    if elevated && kind != "direct" {
        return Err("USB inspection does not require elevation".into());
    }
    let source = if kind == "usb" {
        ...
        let result = if elevated {
            ...
            (|| {
                ...
                let result = if kind == "usb" {
                    ...
                    provider_runtime::verify(&root, &manifest, &cancel)?;
                    let mut request = tempfile::NamedTempFile::new().map_err(|e| e.to_string())?;
                    let protected_paths = vec![
                        source.path.clone(),
                        std::env::current_exe().map_err(|e| e.to_string())?,
                    ];
                    serde_json::to_writer(&mut request, &json!({"protectedPaths":protected_paths}))
                        .map_err(|e| e.to_string())?;
                    request.flush().map_err(|e| e.to_string())?;
                    provider_process::run(
                        provider_process::direct_command(
                            &root,
                            &manifest,
                            "probe",
                            request.path(),
        ...
        match run_elevated(&service, &app, OperationRequest { protocol:1, source, destination }) {
            Ok(receipt) => service.update(&app, |inner| {
                ...
                inner.snapshot.message = if action == "firmware" {
                    "Restart requested. Disable Secure Boot in firmware, leave TPM enabled, save and return to Windows. Reopen this app and check with administrator access."
                } else { "Construction runtime ready. Check with administrator access to continue." }.into();
...
pub fn inspect_usb_choice(
    ...
    let image = usb_inspection_source(&downloads)?;
    let identity = {
        let mut inner = setup
            .inner
            .lock()
...
pub fn start_setup(
    ...
    let source = if usb_mode.as_deref() == Some("restore") {
        usb_inspection_source(&downloads)?
    } else {
        source(&downloads)?
    };
    ...
    if let Some(mode) = usb_mode.as_deref() {
        if !["erase", "preserve", "restore"].contains(&mode) {
        ...
        if mode != "erase" {
            ...
            if (mode == "preserve" && !inspection.eligible)
                || (mode == "restore" && !inspection.restore_available)
            {
        ...
        match value["type"].as_str() {
            Some("confirmation_required") => {
                ...
                let (dialog_kind, button) = match operation {
                    ...
                    "usb_restore" => (MessageDialogKind::Info, "Restore previous boot setup"),
