...
| P2 | Windows PowerShell 5.1 interpreted newly added UTF-8 ellipses as ANSI text, corrupting progress labels. | Keep these provider strings ASCII-compatible. The planner regression now compares the actual PowerShell parser's text against the repository's UTF-8 text and checks syntax for both changed provider scripts. |
